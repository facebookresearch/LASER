...
#--------------------------------------------------------
#
# bash script to train and evaluate classifiers on the Reuters RCV2 corpus
...
lbl="lbl"
txt="txt"
bname="reuters"			# id used is all created files
langs=("de" "en" "fr" "es")	# languages for which training data is available
...
#
# The Reuters corpus doesn't provide a dev corpus
# and we have to tune on the test corpus
###################################################################
...
Train () {
  ...
  if [ ! -f ${logdir}/${fn}.log ] ; then
    python ${mldir}/classify_sent.py \
           --gpu ${gpu} \
           --base_dir ${edir} \
           --train "${bname}.train1000.${txt}.${tok}.join.enc.${ltrn}" --train_labels "${bname}.train1000.${lbl}.${ltrn}" \
           --dev   "${bname}.test.${txt}.${tok}.join.enc.${ldev}" --dev_labels "${bname}.test.${lbl}.${ldev}" \
           --test  "${bname}.test.${txt}.${tok}.join.enc" --test_labels "${bname}.test.${lbl}"  \
           --langs "de" --langs "en" --langs "es" --langs "fr" \
...
Train "de" "de"  8  5 0.4      3615 100
Train "en" "en" 12  4 0.3 123456789 100
Train "es" "es" 16  4 0.4 123456789 100
Train "fr" "fr" 16 20 0.4      3615 100
